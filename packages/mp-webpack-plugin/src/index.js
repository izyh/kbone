        ...
        compiler.hooks.emit.tapAsync(PluginName, (compilation, callback) => {
            ...
            if (wxCustomComponentRoot) {
                ...
                addFile(compilation, '../custom-component/index.wxml', names.map((key, index) => {
                    ...
                    return `<${key} wx:${index === 0 ? 'if' : 'elif'}="{{name === '${key}'}}" id="{{data.id}}" class="{{data.class}}" style="{{data.style}}" ${props.map(name => name + '="{{data.' + name + '}}"').join(' ')} ${events.map(name => 'bind' + name + '="on' + name + '"').join(' ')}><slot/></${key}>`
                }).join('\n'))
